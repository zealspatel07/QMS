// frontend/src/pages/CreateIndent.tsx
...
export default function CreateIndent() {
    ...
    // Extract quotation data from route state (passed from QuotationView)
    const quotationState = location.state as any;
    const quotationData = quotationState?.quotationData;
    ...
    const [productUpdateTimeout, setProductUpdateTimeout] = useState<Record<number, ReturnType<typeof setTimeout>>>({});
    ...
    useEffect(() => {
        const handler = (e: MouseEvent) => {
            ...
            // Only close dropdowns if click is outside customer/vendor/product input fields
            if (!target.closest('[data-customer-field]') && !target.closest('[data-vendor-field]') && !target.closest('[data-product-field]')) {
    ...
    // Pre-fill form with quotation data if quotationId is provided
    useEffect(() => {
        if (quotationData) {
            // Set customer name from quotation
            if (quotationData.customer_name) {
                setCustomerName(quotationData.customer_name);
            }

            // Pre-fill indent date with today's date
            setIndentDate(today);
            ...
            // Set notes from quotation if available
    ...
    async function fetchVendors() {
        try {
            ...
            if (!token) {
                console.warn("No token available, skipping vendor fetch");
                return;
            }
    ...
    function validateForm(): boolean {
        ...
        // ✅ NEW: Check that all items have product_name
    ...
    async function submitIndent(draftMode: boolean = false) {
        if (!validateForm()) {
            return;
        }
        ...
        try {
            ...
            formData.append("status", draftMode ? "draft" : "submitted");
            ...
            await toast.promise(
                api.createIndent(formData),
                {
                    loading: "Creating indent...",
                    success: draftMode ? "✓ Indent saved as draft!" : "✓ Indent created successfully!",
                    error: (err: any) => {
                        const errorMsg = err?.response?.data?.error || err?.message || "Failed to create indent";
                        return errorMsg;
                    }
                }
            );
                ...
                <div className="w-full">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-gray-600 mt-2">
                                    Create a new purchase indent for procurement
                                </p>
                            </div>
                            <button
                    ...
                    <div className="space-y-6">
                                    ...
                                    <table className="w-full">
                                        <thead>
                                            <tr className="border-b border-gray-200 bg-gray-50">
                                                ...
                                                <th className="text-left px-4 py-3 text-sm font-semibold text-gray-700">UOM</th>
                                            ...
                                            {items.map((item, index) => (
                                                <React.Fragment key={index}>
                                                    ...
                                                    <tr className="border-b border-gray-200">
                                                        ...
                                                        {/* PRODUCT + DESCRIPTION */}
                                                        <td className="px-4 py-3" style={{ position: 'relative', zIndex: 10 }}>
                                                            <div className="space-y-2" style={{ position: 'relative', zIndex: 10 }}>

                                                                {/* Product Dropdown - Like Customer */}
                                                                <div className="relative" data-product-field style={{ position: 'relative', zIndex: 50 }}>
                                                                    <input
                                                                        type="text"
                                                                        aria-label="Select or search product"
                                                                        placeholder="Search or select product"
                                                                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white"
                                                                        value={productSearchInput[index] !== undefined ? productSearchInput[index] : (item.product_id ? item.product_name || "" : "")}
                                                                        onChange={(e) => {
                                                                            ...
                                                                            // Update product name
                                                                            updateItem(index, "product_name", newVal);
                                                                            // Update in products list if product_id exists
                                                                            if (item.product_id) {
                                                                                setProducts(prev => prev.map(p => 
                                                                                    p.id === item.product_id ? { ...p, name: newVal } : p
                                                                                ));

                                                                                // Debounce API call to save to database
                                                                                if (productUpdateTimeout[index]) {
                                                                                    clearTimeout(productUpdateTimeout[index]);
                                                                                }
                                                                                const timeout = setTimeout(async () => {
                                                                                    try {
                                                                                        if (item.product_id) {
                                                                                            ...
                                                                                            console.log(`✅ Product ${item.product_id} updated to "${newVal}" with description in database`);
                                                                                        }
                                                                                    } catch (err) {
                                                                                        console.error("Failed to update product in database:", err);
                                                                                    }
                                                                                }, 1000);
                                                                                setProductUpdateTimeout(prev => ({
                                                                                    ...prev,
                                                                                    [index]: timeout
                                                                                }));
                                                                            }
                                                                    ...
                                                                    {/* Product Dropdown - Like Customer Dropdown */}
                                                                    {showProductDropdown[index] && (
                                                                        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-48 overflow-auto z-50">
                                                                            {(() => {
                                                                                const searchTerm = (productSearchInput[index] || item.product_name || "").toLowerCase();
                                                                ...
                                                                <textarea
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        updateItem(index, "product_description", newDesc);
                                                                        
                                                                        // Update in products list if product_id exists
                                                                        if (item.product_id) {
                                                                            setProducts(prev => prev.map(p => 
                                                                                p.id === item.product_id ? { ...p, description: newDesc } : p
                                                                            ));
                                                                            ...
                                                                            const timeout = setTimeout(async () => {
                                                                                try {
                                                                                    if (item.product_id) {
                                                                                        ...
                                                                                        console.log(`✅ Product ${item.product_id} description updated in database`);
                                                                                    }
                                                                                } catch (err) {
                                                                                    console.error("Failed to update product description in database:", err);
                                                                                }
                                                                            }, 1000);
                                                                            setProductUpdateTimeout(prev => ({
                                                                                ...prev,
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <button
                                type="button"
                                onClick={() => submitIndent(true)}
                                disabled={isSubmitting}
                                className="px-6 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
                            >
                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
